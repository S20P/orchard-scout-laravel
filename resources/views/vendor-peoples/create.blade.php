...
<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
    ...
    <li class="breadcrumb-item text-muted">
        <a href="{{route('vendor-peoples.index',$vendor_id)}}" class="text-muted text-hover-primary"> {{__('Vendor Peoples')}}</a>
    </li>
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    ...
    <div id="kt_content_container" class="container-xxl">
        ...
        <div class="card mb-5 mb-xl-10">
            <div id="kt_account_settings_profile_details" class="collapse show">
                <form id="add_frm" name='add_frm' class="form" method="post" action="{{ route('vendor-peoples.store') }}">
                    @csrf
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8 fv-row">
                                <select
                                    ...
                                    <option value=''>{{__('Select people')}}</option>
                                    @if (!empty($Peoples) && count($Peoples) > 0)
                                    @foreach ($Peoples as $People)
                                        <option value='{{$People->id}}' @if (old('people_id')==$People->id)
                                            {{'selected'}}
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8 fv-row">
                                <select
                                    ...
                                    <option value=''>{{__('Select people role')}}</option>
                                    @if (!empty($PeopleRoles) && count($PeopleRoles) > 0)
                                    @foreach ($PeopleRoles as $PeopleRole)
                                        <option value='{{$PeopleRole->id}}' @if (old('people_role_id')==$PeopleRole->id)
                                            {{'selected'}}
...
<script src="{{url('js/module/vendor-peoples.js')}}"></script>
@endsection